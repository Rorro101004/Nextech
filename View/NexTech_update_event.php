...
if (isset($_GET['id_event'])) {
    ...
    $eventData = $eventController->getEventById($_GET['id_event']);
    if (!$eventData) {
            ...
            <?php } else if ($_SESSION["logged"] == true) { ?>
                <div class="profile_logout">
                    <div class="profile_logged">
                        <a href="NexTech_profile.php" style="color:rgb(147, 0, 233)">
                            <?php echo '<img class="profile_image" src="data:image/jpeg;base64,' . base64_encode($_SESSION["profile_image"]) . '" alt="Profile" >' ?>
                        </a>
                    </div>
                    <div class="logout">
                ...
                <div class="forms_create_event">
                    ...
                    <div class="form_create_event">
                        <form action="../Controller/EventController.php" method="post">
                            <input type="hidden" name="id_event" value="<?php echo htmlspecialchars($eventData['id_event']); ?>">
                            <div class="inputs inputs_event">
                                <label>Name</label><br>
                                <input type="text" name="eventName" value="<?php echo htmlspecialchars($eventData['name']); ?>" required><br>
                                <label>Description</label><br>
                                <textarea name="eventDescription" style="height: 50px; width: 300px;" required><?php echo htmlspecialchars($eventData['description']); ?></textarea><br>
                                <label>Start Date</label><br>
                                <input type="date" name="eventStart" value="<?php echo htmlspecialchars($eventData['start_date']); ?>" required><br>
                                <label>End Date</label><br>
                                <input type="date" name="eventEnd" value="<?php echo htmlspecialchars($eventData['end_date']); ?>" required><br>
                                <label>Location</label><br>
                                <input type="text" name="eventLocation" value="<?php echo htmlspecialchars($eventData['location']); ?>" required><br>
                                <label>Price</label><br>
                                <input type="number" name="eventPrice" value="<?php echo htmlspecialchars($eventData['price']); ?>" required><br>
                                <label>Url</label><br>
                                <input type="url" name="eventUrl" value="<?php echo htmlspecialchars($eventData['url']); ?>" required><br>
                            </div>
                            <div>
